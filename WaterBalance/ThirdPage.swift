...
struct ThirdPage: View {
    @State var textValue: String = "2400"
    ...
    var body: some View {
        VStack{
            ...
            Spacer()
            Spacer()
            Buttona(inbutton: "Save",destination: AnyView(FourthPage(textValue: $textValue)))
    ...
    var body: some View {
        ...
        VStack {
            ZStack {
                ...
                VStack(spacing: 58) {
                    CircleWaveView(textValue: $textValue, percentt: $percent, percent: Int(self.percent)+Int((Double(textValue)! / 1000)/2.4 * 100))
    ...
    var body: some View {
        ...
        GeometryReader { geo in
            ZStack {
                ...
                Text("\(Int((Double(textValue)! / 1000)/2.4 * 100)) %")
                    .foregroundColor(.black)
                //                    .font(Font.system(size: 0.25 * min(geo.size.width, geo.size.height) ))
                    .padding(.bottom,20)
                    .font(.system(size: 36,weight: .bold))

//                Text("\(floor(Double(textValue)!*10)/10.0) out of 2,4 L")
                Text("\(String(format: "%.1f", (Double(textValue)!) / 1000)) out of 2,4 L")
//                floor(value * 10) / 10.0
                    .font(.system(size: 16,weight: .medium))
                    .padding(.top,60)
                    .foregroundColor(.white)
